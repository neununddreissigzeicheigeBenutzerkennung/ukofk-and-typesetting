%%% This package aims to provide a versitile \list control sequence which takes custom \lbullet's (#1) and then takes care of spacing for the \item's in #2. For more detailled information view the README file.
%%% A  few extra control sequences where used to achive \list's functionality which if not helpfull may at least be interesting to some and are thus also present in the README file.
